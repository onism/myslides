...
%        Signal Processing, IEEE Transactions on. 2007, 55(7) pp3558 Section IV
%
...
%% Set Global matrix for fast calculation.
global JJgl;
global KKgl;
global NNgl;
global WWgl;
global LLgl;
UIdx = triu(repmat(1:nCardMax,   [nCardMax,1]),0);
RIdx = triu(repmat((1:nCardMax)',[1,nCardMax]),0);
JJgl = zeros((nCardMax+1)*(nCardMax)/2,nCardMax);
KKgl = zeros((nCardMax+1)*(nCardMax)/2,nCardMax);
NNgl = zeros((nCardMax+1)*(nCardMax)/2,nCardMax);
for n=0:nCardMax-1
    MatIdx = UIdx(1:n+1,:)';  % Use matrix operation for speed.
    JJ     = MatIdx(MatIdx>0);
    JJgl(1:length(JJ),n+1) = JJ;
    MatIdx = RIdx(1:n+1,:)';
    KK     = MatIdx(MatIdx>0);
    KKgl(1:length(KK),n+1) = KK;
    NNgl(:,n+1) = ones((nCardMax+1)*(nCardMax)/2,1)*n;
end
JJ       = JJgl-1;
KK       = KKgl-1;
JJ(JJ<0) = 0;
...
JJgl     = JJ;
KKgl     = KK;
WWgl     = factorial(JJ)./factorial(JJ-KK)./factorial(KK);
clear UIdx RIdx JJ KK;
...
% %         end
        ...
        %% Start CPHD Filtering Routine.
        Z = ZCLobsv{k+1,m}; 
%         ProbBirth = poisspdf(0:nCardMax,4);
        ProbBirth = [ones(1,20)/20 zeros(1, nCardMax-20)]; %ones(1,nCardMax)/nCardMax;
...
%         end
        ...
        % XCardHat(k+1,m)     = size(XCLFilter{k+1,m},2);%(XCardHat(k+1,m) + ;
...
plot(2:nSims, XCardHat(2:end, m), 'ko');
xlabel('time step');
ylabel('n_T');
